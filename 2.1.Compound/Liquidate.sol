...
contract CompoundLiquidator {
  ...
  // close factor - % of borrow that can be liqudated (scaled up by le18)
  function getCloseFactor() external view returns (uint) {
    return comptroller.closeFactorMantissa(); // to get in %, divide by le18
  }
  

  // liquidation incentive -> discounted tokens recieved as an intencive to liquidate a user
  ...
  function getAmountToBeLiquidated(address _cTokenBorrowed, address _cTokenCollateral, uint _actualRepayAmount) external view returns (uint) {
    /*
     ...
     */
    (uint error, uint cTokenCollateralAmount) = comptroller.liquidateCalculateSeizeTokens(_cTokenBorrowed, _cTokenCollateral, _actualRepayAmount);
